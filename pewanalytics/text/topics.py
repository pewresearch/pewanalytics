...
class TopicModel(object):
    ...
    def fit(self, df=None, **kwargs):
        ...
        **sklearn_lda**
        ...
        topics; when values are lower, documents will be primarily comprised of only a few topics. This parameter \
        is used instead of the doc_topic_prior sklearn parameter, and will be passed along to sklearn using the \
        formula: `doc_topic_prior = alpha / num_topics`
        ...
        **gensim_lda**
        ...
        :py:class:`gensim.models.ldamulticore.LdaMulticore`. When `use_multicore` is set to `True`, the multicore \
        implementation will be used, otherwise the standard LDA implementation will be used. For more information on \
        ...
        :param df: The dataframe to train the model on (must contain `self.text_col`)
        :param alpha: Represents document-topic density. When values are higher, documents will be comprised of more \
        topics; when values are lower, documents will be primarily comprised of only a few topics. Gensim options are \
        a bit different than sklearn though; refer to the documentation for the accepted values here.
        :param beta: Represents topic-word density. When values are higher, topics will be comprised of more words; \
