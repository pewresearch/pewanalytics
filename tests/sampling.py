...
class SamplingTests(unittest.TestCase):
    ...
    def test_compute_sample_weights_from_frame(self):
        ...
        self.assertTrue([0.096, 1.904] == weights)
    ...
    def test_compute_balanced_sample_weights(self):
        ...
        weights = sorted([round(w, 3) for w in weights.unique()])
        self.assertTrue([0.515, 16.667] == weights)
    ...
    def test_sample_extractor(self):
        ...
        frame_props = (self.df.groupby(["flag", "flag2"]).count() / len(self.df))[
            "id"
        ].to_dict()
        ...
        for k, v in frame_props.items():
            ...
            self.assertTrue(sample_props[k] < frame_props[k] + 0.03)
            self.assertTrue(sample_props[k] > frame_props[k] - 0.03)
        ...
        self.assertTrue(np.average(list(sample_props.values())) == 0.25)
        ...
        for k, v in sample_props.items():
            self.assertTrue(sample_props[k] > 0)
            self.assertTrue(sample_props[k] < frame_props[k] + 0.01)
            self.assertTrue(sample_props[k] > frame_props[k] - 0.01)
        ...
        self.assertTrue(len(normal_strat) == 100)
        sample_props = (
            self.df.loc[strat_alt].groupby(["flag", "flag2"]).count() / len(strat_alt)
        )["id"].to_dict()
        for k, v in sample_props.items():
            self.assertTrue(sample_props[k] > 0)
            self.assertTrue(sample_props[k] < frame_props[k] + 0.01)
